...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="The filename format for downloaded tracks. Default is '{}'. All placeholders: {}".format(
            DEFAULT_FILENAME_FORMAT, ", ".join(TRACK_INFO_KEYS)
        ),
    )
    parser.add_argument(
        "--format",
...
def download_album(_album_url: str, _attempt: int = 1) -> None:
    try:
        ...
        if _attempt < CONFIG["MAX_URL_ATTEMPTS"]:
            if CONFIG["VERBOSE"] >= 2:
                CONFIG["TQDM"].write(
                    "WARN: I/O Error on attempt # [{}] to download the album at [{}]. Trying again...".format(
                        _attempt, _album_url
                    )
                )
...
def download_file(_url: str, _track_info: dict = None, _attempt: int = 1) -> None:
    try:
        with requests.get(
            ...
            if expected_size != actual_size:
                raise IOError(
                    "Incomplete read. {} bytes read, {} bytes expected".format(
        ...
        if _attempt < CONFIG["MAX_URL_ATTEMPTS"]:
            if CONFIG["VERBOSE"] >= 2:
                CONFIG["TQDM"].write(
                    "WARN: I/O Error on attempt # [{}] to download the file at [{}]. Trying again...".format(
                        _attempt, _url
                    )
                )
            time.sleep(CONFIG["URL_RETRY_WAIT"])
